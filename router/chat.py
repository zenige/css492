...
async def webhook(payload: Dict):
    ...
    if message_type == 'text':
        ...
        res = basicState.basicEventHandler(
            sender_id=sender['userId'], message=data, confident=0.6,reply_token=Reply_token,userName=profile.display_name)
    ...
    if "message" in res.keys():
        print('in     if "message" in res.keys():')
    ...
    elif 'img' in res.keys():
        response = ImageSendMessage(
        original_content_url=res['img'],
        preview_image_url=res['img'])
        ...
        original_content_url=i['img'],
        preview_image_url=i['img']))
